...
import { Check, X, CheckCheck, Inbox, ArrowLeftRight, Store } from "lucide-react";
...
export function ConfirmationsPage() {
  ...
  const resolveAll = useStore((s) => s.resolveAllForAccount);
    ...
    <div className="mx-auto max-w-3xl p-4 sm:p-6">
      ...
      <div className="mt-5 flex gap-1 rounded-xl bg-surface-sunken p-1">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            <span className="rounded-full bg-surface-sunken px-1.5 text-xs text-ink-faint">
              {counts[t.id]}
            </span>
          </button>
        ))}
      ...
      {grouped.size === 0 ? (
        <div className="card mt-6 flex flex-col items-center gap-3 p-12 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-ink-faint">No pending confirmations right now.</p>
          </div>
        </div>
      ) : (
            ...
            return (
              <section key={accountId}>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => resolveAll(accountId, "allow")}
                ...
                <div className="space-y-2.5">
                  {items.map((c) => (
                    <ConfirmationCard key={c.id} conf={c} onResolve={resolve} />
                  ))}
                </div>
              </section>
            );
...
function ConfirmationCard({
  ...
  conf: Confirmation;
  onResolve: (id: string, action: "allow" | "cancel") => void;
}) {
    ...
    <div className="card flex items-center gap-3 p-3 sm:p-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 text-xs text-ink-faint">
          <span className="truncate">{conf.subtitle}</span>
          <span>·</span>
      ...
      <div className="flex shrink-0 gap-2">
        <button
          onClick={() => onResolve(conf.id, "allow")}
        ...
        <button
          onClick={() => onResolve(conf.id, "cancel")}
